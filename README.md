...
- [x] Explain what a token is used for.
      A token is a string of data sent by a server to a client when the user authenticates herself or himself with the correct credentials.
      Typically a token needs to be retrieved to grant users access to data/resource from the app, this practice helps secure a website as well as protects the users sensitive information.
- [x] What steps can you take in your web apps to keep your data secure?
      Set up a strong authentication system, protect sensitive data, implement a strong password system, implementing tests to find system vulnerabilities, etc...
- [x] Describe how web servers work.
      Web servers are 2 things technically, it is the dedicated computer as well as the program running that computer which stores specific information. This web server is connected to the world wide web and can be located through a specific URL and domain name which clients then use to communicate with this particular server to request data/resource. 
